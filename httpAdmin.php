...
//print_r($searchApi->search($searchBase,'[object2]'));

//用中括号包裹,并且内部有等于判断,代表查找包含某个属性的元素,并且元素的值等于=后面,[className=phpInterpreter]就代表包含className属性,并且值是phpInterpreter的元素
//print_r($searchApi->search($searchBase,'[className=phpInterpreter]'));

//当然也可以复合使用,会类似jquery从父层往下查去匹配
//print_r($searchApi->search($searchBase,'.= .new'));//查找type='='里的type='new'

//直接输入字符查找,下面例子中的object2,带包直接进入object2作为命中
//print_r($searchApi->search($searchBase,'.= object2'));//查找type='='里的object2

//filter过滤器,对查出来的结果进行过滤.判断是否满足filter()括号里包含的特征
//print_r($searchApi->search($searchBase,'.= object2:filter([className=phpInterpreter])'));
